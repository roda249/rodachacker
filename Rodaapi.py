...
# -*- coding: utf-8 -*-
"""
RODA - YEŞİL TEMA (TÜM ÖZELLİKLER)
Admin/Üye | Kalıcı Key'ler | Hit Arşivi Silme | Loglar
"""
...
# ============================================================
# KEY YÖNETİMİ (KALICI - OTOMATİK SİLME YOK)
# ============================================================
def load_keys():
    ...
    if key in keys:
        ...
        if exp:
            if datetime.now() < datetime.fromisoformat(exp):
                ...
                # SÜRE DOLSA BİLE SİLME - SADECE MANUEL SİLİNİR
                return False, None
        else:
            return True, entry.get("note", "Kullanıcı")
    return False, None
...
:root{--p:#00e676;--p2:#00bcd4;--g:#00e676;--r:#ff5252;--card:#12192e;--border:rgba(0,230,118,0.12);--bg:#0a0e1a;--sidebar:#060a16;--text:#e8edf5;--muted:#8a9bb0;--gold:#ffd740}
...
#login-box{width:400px;padding:45px 40px;text-align:center;background:var(--card);border:1px solid var(--border);border-radius:28px;box-shadow:0 20px 50px rgba(0,230,118,0.06)}
...
.inp:focus{border-color:var(--p);box-shadow:0 0 20px rgba(0,230,118,0.06)}
...
.btn:hover{transform:translateY(-2px);box-shadow:0 8px 30px rgba(0,230,118,0.2)}
...
.nav-item:hover{background:rgba(0,230,118,0.05);color:#fff}
.nav-item.active{background:rgba(0,230,118,0.1);color:var(--p);border-left:3px solid var(--p)}
...
.result-row:hover{background:rgba(0,230,118,0.02)}
...
.cat-api{background:rgba(0,230,118,0.12);color:var(--p)}
...
.checker-platform-select button{padding:6px 14px;background:rgba(0,230,118,0.06);border:1px solid rgba(0,230,118,0.12);border-radius:8px;color:#8a9bb0;font-size:12px;cursor:pointer;transition:0.2s;display:flex;align-items:center;gap:4px}
.checker-platform-select button:hover{background:rgba(0,230,118,0.12);border-color:var(--p);color:#fff}
.checker-platform-select button.active{background:rgba(0,230,118,0.15);border-color:var(--p);color:var(--p)}
...
.discovery-platforms button{padding:4px 12px;background:rgba(0,230,118,0.06);border:1px solid rgba(0,230,118,0.1);border-radius:6px;color:#8a9bb0;font-size:11px;cursor:pointer;transition:0.2s}
.discovery-platforms button:hover{background:rgba(0,230,118,0.1);border-color:var(--p);color:#fff}
.discovery-platforms button.active{background:rgba(0,230,118,0.12);border-color:var(--p);color:var(--p)}
...
.parse-tabs button{padding:6px 16px;background:rgba(0,230,118,0.06);border:1px solid rgba(0,230,118,0.12);border-radius:8px;color:#8a9bb0;font-size:12px;cursor:pointer;transition:0.2s}
.parse-tabs button:hover{background:rgba(0,230,118,0.12);border-color:var(--p);color:#fff}
.parse-tabs button.active{background:rgba(0,230,118,0.15);border-color:var(--p);color:var(--p)}
.logs-table{width:100%;border-collapse:collapse;font-size:12px}
.logs-table th{text-align:left;padding:8px 12px;background:rgba(0,230,118,0.08);color:var(--p);font-weight:600;border-bottom:2px solid var(--border)}
...
.hit-clear-btn{background:var(--r);color:#fff;border:none;padding:4px 12px;border-radius:6px;cursor:pointer;font-size:11px;margin-left:8px}
.hit-clear-btn:hover{opacity:0.8}
::-webkit-scrollbar{width:4px}::-webkit-scrollbar-thumb{background:rgba(0,230,118,0.15);border-radius:4px}
</style>
</head>
<body>
<div id="login-screen">
...
<div id="sidebar">
<div class="sidebar-header"><div class="logo-text">RODA</div><div class="version">v5.0</div></div>
...
<div class="nav-item admin-only" data-page="discovery" onclick="switchPage('discovery')"><i class="fa-solid fa-compass"></i> API Keşif</div>
...
<div class="nav-item admin-only" data-page="keys" onclick="switchPage('keys')"><i class="fa-solid fa-key"></i> Key Yönetimi</div>
<div class="nav-item admin-only" data-page="logs" onclick="switchPage('logs')"><i class="fa-solid fa-history"></i> Loglar</div>
</div>
...
<span id="userBadge" style="font-size:11px;background:var(--p);color:#fff;padding:2px 12px;border-radius:12px;display:none">Admin</span>
</div>
</div>
...
<select id="hitPlatformFilter" onchange="renderHits()"><option value="all">Tüm Platformlar</option></select>
<button class="hit-clear-btn" onclick="clearAllHits()">Tümünü Sil</button>
</div>
<div class="hit-panel">
...
<div id="page-discovery" class="page admin-only">
...
<div id="page-keys" class="page admin-only">
<div class="card">
...
<!-- LOGLAR (SADECE ADMIN) -->
<div id="page-logs" class="page admin-only">
<div class="card">
...
function clearAllHits() {
    if (!isAdmin) return;
    if (!confirm("Tüm hit'leri silmek istediğinize emin misiniz?")) return;
    fetch("/api/admin/clear_hits", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
if __name__ == "__main__":
    ...
    ║     ✅ Key Süresi: Dakika/Saat/Gün (Kalıcı)                  ║
    ║     ✅ Hit/2FA Arşivi Silme                                  ║
    ║     ✅ Valorant API Aktif                                    ║
    ║     ✅ Login Düzeltildi                                      ║
    ╚══════════════════════════════════════════════════════════════════╝
    """)
